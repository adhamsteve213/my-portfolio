...
const API_BASE_URL = process.env.REACT_APP_API_URL || 'https://portfolio-back-main-47f9772a6221.herokuapp.com/api';
const TOKEN_KEY = 'portfolio_admin_token';
const ADMIN_EMAIL = process.env.REACT_APP_ADMIN_EMAIL || 'admin@portfolio.test';
...
const AdminPanel = () => {
  ...
  const [isPasswordVerified, setIsPasswordVerified] = useState(
    sessionStorage.getItem('portfolio_admin_verified') === 'true'
  );
  ...
  const handleLogin = async (event) => {
    ...
    sessionStorage.setItem('portfolio_admin_verified', 'true');
    setPassword('');
  ...
  const handleLogout = async () => {
    ...
    sessionStorage.removeItem('portfolio_admin_verified');
    setToken('');
